...
class AuthGuard {
  ...
  static bool get isGuest {
    try {
      ...
      if (sessionController.isDemoMode.value) return false; // Demo Mode is treated as Authenticated
  ...
  static bool get isAuthenticated {
    try {
      final sessionController = Get.find<AppSessionController>();
      if (sessionController.isDemoMode.value) return true; // Demo Mode is Authenticated
  ...
  static bool canAccess(String featureName) {
    try {
      final sessionController = Get.find<AppSessionController>();
      if (sessionController.isDemoMode.value) return true; // Unlock all in Demo
    } catch (_) {}
  ...
  static void requireAuth({
    ...
    try {
      final sessionController = Get.find<AppSessionController>();
      if (sessionController.isDemoMode.value) {
        onAllowed();
        return;
      }
    } catch (_) {}
    ...
    if (canAccess(featureName)) {
      ...
      try {
        ...
        if (Get.currentRoute.isNotEmpty) {
          sessionController.setLastIntendedRoute(Get.currentRoute);
        }
      } catch (_) {}
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            ...
            // Subtitle
            Text(
              'Fitur $featureName butuh akun biar progres dan catatanmu aman tersimpan.',
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(
                  ...
                  side: const BorderSide(color: MuamalahColors.glassBorder),
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(16),
                  ),
                ),
